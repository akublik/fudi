...
const complementaryDishSuggestionPrompt = ai.definePrompt({
  ...
  input: {schema: ComplementaryDishSuggestionInputSchema.extend({ isGourmet: z.boolean(), isFryer: z.boolean(), isParrillada: z.boolean(), isBebida: z.boolean(), isReposteria: z.boolean() })},
...
{{else if isBebida}}
Please provide suggestions for beverages (cocktails, juices, etc.) that pair well with the main course.
{{else if isReposteria}}
Please provide suggestions for desserts that would complement the main course.
{{else}}

...
const IngredientBasedRecipeSuggestionInputSchema = z.object({
  ...
  style: z.enum(['Sencillo', 'Gourmet', 'Fryer', 'Parrillada', 'Bebidas', 'Reposteria']).describe('The desired cooking style.'),
...
const prompt = ai.definePrompt({
  ...
  input: {schema: IngredientBasedRecipeSuggestionInputSchema.extend({ isGourmet: z.boolean(), isFryer: z.boolean(), isParrillada: z.boolean(), isBebida: z.boolean(), isReposteria: z.boolean() })},
...
Please provide recipes that are ideal for a barbecue or cookout (parrillada). They should be suitable for grilling.
{{else if isBebida}}
Please provide recipes for beverages (cocktails, juices, smoothies, etc.) using the provided ingredients.
{{else if isReposteria}}
Please provide recipes for desserts, pastries, or breads using the provided ingredients.
{{else}}
...
const ingredientBasedRecipeSuggestionFlow = ai.defineFlow(
  ...
  async input => {
    ...
    if (!output || !output.recipes || output.recipes.length === 0) {
      console.warn('No recipes were generated by the prompt.');
      return { recipes: [] };
    }
